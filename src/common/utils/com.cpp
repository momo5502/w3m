...
namespace utils::com
{
    namespace
    {
        class ComInitializer
        {
          public:
            ComInitializer()
                : succeeded_(SUCCEEDED(CoInitializeEx(nullptr, COINIT_MULTITHREADED)))
            {
            }
        ...
        void initialize_com()
        {
            thread_local std::optional<ComInitializer> initializer{};
            if (!initializer)
            {
                initializer.emplace();
            }
    ...
    bool select_folder(std::string& out_folder, const std::string& title, const std::string& selected_folder)
    {
        ...
        const auto _ = finally([raw_path]() { CoTaskMemFree(raw_path); });
