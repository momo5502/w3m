...
namespace network
{
	...
	{
		thread_ = utils::thread::create_named_jthread("Network Dispatcher", [this](const std::stop_token& stop_token)
		{
			this->packet_receiver(stop_token);
		});
	}
	...
	{
		this->callbacks_.access([&](callback_map& callbacks)
		{
