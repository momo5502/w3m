...
#define UPDATE_FILE_MAIN   UPDATE_SERVER "w3.json"
#define UPDATE_FOLDER_MAIN UPDATE_SERVER "w3/"

#define UPDATE_HOST_BINARY "w3m.exe"
    ...
    void file_updater::update_files(const std::vector<file_info>& outdated_files) const
    {
        ...
        for (size_t i = 0; i < thread_count; ++i)
        {
            threads.emplace_back([&]() {
                while (!exception.access<bool>([](const std::exception_ptr& ptr) { return static_cast<bool>(ptr); }))
                {
                    ...
                    {
                        exception.access([](std::exception_ptr& ptr) { ptr = std::current_exception(); });
    ...
    void file_updater::cleanup_data_directory(const std::vector<file_info>& files) const
    {
        ...
        for (auto& file : existing_files)
        {
            ...
            if (is_file || is_folder)
            {
                ...
                {
                    if ((is_folder && is_inside_folder(legal_file, file)) || (is_file && legal_file == file))
                    {
                        is_legal = true;
                        break;
                    }
